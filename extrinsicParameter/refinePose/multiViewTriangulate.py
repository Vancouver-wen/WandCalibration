...
def multi_view_triangulate(
    ...
    if solve_method=="SVD":
        ...
        # 对 D.T@D 最小特征值对应的特征向量就是 超定方程的最小二乘解
...
def normalized_pole_triangulate(
    ...
    for normalized_pole_list in normalized_pole_lists:
        ...
        masked_pose_list=[
            poses[temp[1]]
            for temp in filter(lambda x:x[0],list(zip(mask,sorted(poses))))
        ]
        point_3ds=[]
        # temp=np.divide(
        #     np.abs(masked_pole_list[0]-masked_pole_list[1]),
        #     np.abs(masked_pole_list[1]-masked_pole_list[2])
        # )
        # import pdb;pdb.set_trace() # 应该有大致的比例
